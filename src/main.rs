...
const WEBSOCKET_STRING: &str ="258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
...
//Client WebSocket Implementation using HttpParser.
...
//generate http websocket key
fn gen_key(key: &String) -> String {
    let mut m = sha1::Sha1::new();
    let mut buf = [0u8; 20];

    m.update(key.as_bytes());
    m.update(WEBSOCKET_STRING.as_bytes());
    m.output(&mut buf);
...
//Implementation of the Server Websocket Interface using MIO
